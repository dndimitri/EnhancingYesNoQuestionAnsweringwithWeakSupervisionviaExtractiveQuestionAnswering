...
class RobertaMethodClassifier(nn.Module):

    def __init__(self, dropout=0.5):

        super(RobertaMethodClassifier, self).__init__()
        self.bert = RobertaModel.from_pretrained('models/roberta')
        self.qa_outputs = nn.Linear(768, 2)
        self.dropout = nn.Dropout(dropout)
        self.linear = nn.Linear(768, 2)

    def forward(self, qa, qa_masks, binary_output=None, start_positions=None, end_positions=None, mode='train'):
