...
trainer = utils.Trainer(train_dataloader,device,'bert_base')
with open(f'results/bert_base_{batch_size}_batch_size.csv','w') as f:
    ...
    for model,learning_rate,seed,total_loss_train,total_train_acc,epoch in trainer.train():
        validator = utils.Validator(val_dataloader,device,model,'bert_base')
        ...
        with open(f'test_results/bert_base_{learning_rate}_{seed}_{epoch}.csv','w') as f2:
            output = testing.test()
